...
<header class="bg-navy border-b border-primary sticky top-0 z-50" x-data="{ mobileOpen: false, langOpen: false }">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                @else
                    <a href="{{ localized_route('login') }}"
                       @class([
                           ...
                           'text-white hover:bg-primary/40' => ! request()->routeIs('login'),
                       ])>
                        {{ __('auth.login_nav') }}
                    </a>
                @endauth
        ...
        <nav x-show="mobileOpen" x-cloak class="md:hidden py-4 border-t border-primary" aria-label="Mobile">
            ...
            @else
                <a href="{{ localized_route('login') }}" class="block py-2 px-4 text-sm font-medium rounded-lg mb-1 text-white hover:bg-primary/40" @click="mobileOpen = false">
                    {{ __('auth.login_nav') }}
                </a>
            @endauth
